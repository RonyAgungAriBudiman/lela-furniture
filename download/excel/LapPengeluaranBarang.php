...
$inputFileName = 'template/LapPengeluaranBarang.xls';
...
$outputFileName = 'Laporan Pengeluaran Barang Per Dokumen Pabean.xls';
...
foreach ($data as $row) {
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('E' . $baris, $row["DO"]);
    $objPHPExcel->getActiveSheet()->setCellValue('F' . $baris, date("d-M-Y", strtotime($row['tglDO'])));
    $objPHPExcel->getActiveSheet()->setCellValue('G' . $baris, $row["customer"]);
